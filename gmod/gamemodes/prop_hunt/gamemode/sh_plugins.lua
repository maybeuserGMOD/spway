...
hook.Add("Initialize", "PHE.InitPlugins", function() PHE:InitializePlugin() end)
	...
	hook.Add("PH_CustomTabMenu", "PHE.NewPlugins", function(tab, pVgui)
		...
		if table.Count(PHE.PLUGINS) < 1 then
			...
			for plName, Data in pairs(PHE.PLUGINS) do
				...
				if (LocalPlayer():IsSuperAdmin() or table.HasValue(PHE.SVAdmins, LocalPlayer():GetUserGroup())) and table.Count(Data.settings) > 0 then
