...
function PANEL:Think()
	...
	if self.PlayerCount ~= Count then
		...
		self.TeamName:SetText(team.GetName(self.iTeamID) .. " " .. string.format(PHE.LANG.DERMA[Count == 1 and "PLAYER" or "PLAYERS"], self.PlayerCount))
	end
...
function PANEL:UpdateColumn(i, col, pLine)
	...
	if col.UpdateRate == 0 and pLine.UpdateTime[i] ~= 0 then return end -- 0 = only update once
	...
	if IsValid(lbl) and not lbl.bScorePanelHooks then
...
function PANEL:CleanLines(pLine)
	for k, line in pairs(self.List.Lines) do
		if not IsValid(line.pPlayer) or line.pPlayer:Team() ~= self.iTeam then
